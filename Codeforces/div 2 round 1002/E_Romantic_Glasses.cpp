...
void solve() {
    ...
    set<ll> s;
    s.insert(0);
    //let's say we add all odd and subtract all evens
    //and we keep storing these numbers into the map
    //if any subarray sum will be 0 then then array before it should be present in the set
    ll sum = 0;
    for(int i = 0; i<n; i++){
        if(i%2){    
            sum += a[i];
