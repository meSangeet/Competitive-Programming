...
// Solve Function
double func(map<ll,ll> m1, map<ll,ll> m2, double siz1, double siz2){
    double ans = 0;
    for(auto x : m1){
        double num1 = x.first;
        double freq1 = x.second;
        double p1 = freq1/siz1;
        // cout<<p1<<endl;
        double freq2 = m2[num1];
        double p2 = freq2/siz2;
        // cout<<p1<<" "<<p2<<endl;
        double add = p1 * p2;
        // cout<<add<<endl;
        ans += add;
...
void solve() {
    ...
    for(int i = 0; i < n; i++){
        ...
        for(int j = i+1; j<n; j++){
            auto m2 = vec[j];
            double curAns = func(m1.second, m2.second, m1.first, m2.first);
            ans = max(ans, curAns);
        }
    }
